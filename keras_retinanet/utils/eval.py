...
def _get_detections(generator, model, score_threshold=0.05, max_detections=100, save_path=None):
    ...
    for i in progressbar.progressbar(range(generator.size()), prefix='Running network: '):
        ...
        if save_path is not None:
            draw = raw_image.copy()
            draw_annotations(draw, generator.load_annotations(i), label_to_name=generator.label_to_name)
            draw_detections(draw, image_boxes, image_scores, image_labels, label_to_name=generator.label_to_name, score_threshold=score_threshold)
...
def evaluate(
    ...
    for label in range(generator.num_classes()):
        ...
        if num_annotations == 0:
            ...
            continue
        print("flase_positives",false_positives)
